import {
	EntidadNegocio,
	Domicilio,
} from '../models/datos.entidad.negocio.domicilios.model.js';
import { ContactoEntidad } from '../models/orgContactoEntidad.model.js';
...
const crearIdEmpresa = async (req, res) => {
	try {
		...
		// console.log(propiedadesEntidad);
		const resultEntidad = await EntidadNegocio.create(entidad);
		const resultDomcilio = await Domicilio.create(domicilio);
...
const desactivarRFC = async (req, res) => {
	try {
		...
		return res.status(200).json({
			message: 'Entidad de negocio desactivada: ' + data.EntidadNegocioId,
		});
	} catch (error) {
		console.error('Error al desactivar entidad de negocio:', error.message);
		res.status(500).json({ success: false, error: 'Internal Server Error' });
...
const obtenerDescripcion = async (req, res) => {
	try {
		...
		console.log(
			'Error al obtener la descripcion del regimen fiscal',
			error.message,
		);
		res.status(500).json({ error: 'Internal Server Error' });
	}
...
const buscarDescripcion = async (req, res) => {
	try {
		...
		console.log(
			'Error al buscar la descripcion del regimen fiscal',
			error.message,
		);
		res.status(500).json({ error: 'Internal Server Error' });
	}
...
const crearDescripcion = async (req, res) => {
	try {
		...
		console.log(
			'Error al crear la descripcion del regimen fiscal',
			error.message,
		);
		res.status(500).json({ error: 'Internal Server Error' });
	}
...
const desactivarDescripcion = async (req, res) => {
	try {
		...
		return res.status(200).json({
			message: 'Regimen fiscal desactivado: ' + data.ClaveRegimenFiscal,
		});
	} catch (error) {
...
const obtenerDatosContactoxEmpresa = async (req, res) => {
	try {
		const id = req.params.id;
